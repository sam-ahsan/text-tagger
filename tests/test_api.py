...
def wait_for_success(client, job_id: str, timeout: float = 5.0, interval: float = 0.5):
    ...
    while time.monotonic() < deadline:
        response = client.get(f"/v1/tag/batch/{job_id}")
...
def test_tag_endpoint(client):
    ...
    response = client.post("/v1/tag", json=payload)
    assert response.status_code == 200
    data = response.json()
    assert "results" in data and len(data["results"]) == 2
    assert data["results"][1]["topics"] is not None
    
def test_batch_submit_and_status(client):
    payload = {
        "texts": ["Elon Musk visited Berlin.", "NVIDIA announced new GPUs."],
        "language": "en",
        "domain_dict": ["technology", "AI"]
    }
    response = client.post("/v1/tag/batch", json=payload)
    assert response.status_code == 200
    job_id = response.json()["job_id"]

    body = wait_for_success(client, job_id)
